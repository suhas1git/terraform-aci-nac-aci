...
| <a name="input_tag_annotations"></a> [tag\_annotations](#input\_tag\_annotations) | List of tagAnnotation children on fvAEPg. Keys must be unique; `value` is optional in NAC YAML—omit or pass "" for an empty tagAnnotation value. | <pre>list(object({<br/>    key   = string<br/>    value = optional(string, "")<br/>  }))</pre> | `[]` | no |
