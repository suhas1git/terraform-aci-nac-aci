...
| <a name="input_selectors"></a> [selectors](#input\_selectors) | List of selectors. ALlowed values `type`: `all`, `range`. Default value `type`: `range`. Allowed values `from`: 0-255. Allowed values `to`: 0-255. | <pre>list(object({<br/>    name         = string<br/>    policy_group = optional(string)<br/>    type         = optional(string, "range")<br/>    pod_blocks = optional(list(object({<br/>      name = string<br/>      from = number<br/>      to   = optional(number)<br/>    })), [])<br/>  }))</pre> | `[]` | no |
